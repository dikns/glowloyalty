...
app.get('/api/customer/profile', requireAuth, async (req, res) => {
  const { data: user, error } = await supabase
    .from('users')
    .select('id, name, email, phone, role, points, tier, qr_token, created_at')
    .eq('id', req.user.id)
    .single();
...
app.get('/api/customer/visits', requireAuth, async (req, res) => {
...
// ── Staff Routes ──────────────────────────────────────────────────────────────
app.get('/api/staff/scan/:qrToken', requireStaff, async (req, res) => {
  const { data: user } = await supabase
    .from('users')
...
app.get('/api/staff/customer/:id/visits', requireStaff, async (req, res) => {
  const { data: visits } = await supabase
    .from('visits')
    .select('*, staff:staff_id(name)')
...
app.post('/api/staff/visit', requireStaff, async (req, res) => {
  ...
  await supabase.from('users').update({ points: newPoints, tier: newTier }).eq('id', customer_id);
...
app.get('/api/staff/analytics', requireStaff, async (req, res) => {
...
async function start() {
  ...
  const { data: staffExists } = await supabase
    .from('users').select('id').eq('role', 'staff').limit(1).single();

  if (!staffExists) {
    const hash = await bcrypt.hash('osebje123', 10);
    const { error } = await supabase.from('users').insert({
      name: 'Admin Osebje',
      email: 'osebje@salon.si',
